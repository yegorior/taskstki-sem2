...
bool Rectangle::isRectangle() const
{
    ...
    bool allSidesExist = sideOne > EPS_VALUE && sideTwo > EPS_VALUE &&
                         sideThree > EPS_VALUE && sideFour > EPS_VALUE;

    bool oppositeSidesEqual = fabs(sideOne - sideThree) < EPS_VALUE &&
                              fabs(sideTwo - sideFour) < EPS_VALUE;

    bool diagonalsEqual = fabs(diagonalOne - diagonalTwo) < EPS_VALUE;
...
void Rectangle::sortPoints()
{
    ...
    sort(rectanglePoints.begin(), rectanglePoints.end(), [middlePoint](const Point& leftPoint, const Point& rightPoint)
    {
        ...
        if (leftAngle < 0)
        {
            leftAngle += 2 * PI_VALUE;
        }

        if (rightAngle < 0)
        {
            rightAngle += 2 * PI_VALUE;
        }
